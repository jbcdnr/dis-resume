...
\begin{itemize}
  ...
  \item For wildcards and alternate labels the label in the data graph has to be contained in the set of labels specified by these extended schema edge labels: if x$_{1}\rightarrow_{l}$ y$_{1}$ then x$_{2}\rightarrow_{l}$ y$_{2}$ or x$_{2}\rightarrow_{\_}$ y$_{2}$ or x$_{2}\rightarrow_{l|k|...}$ y$_{2}$.  We require that the type of a data value stored in the data graph stores to the type label of the node in the schema graph this node is related to.
...
For each data guide node constructed and for each label of an  edge leaving in the data graph some element from the data guide node:
\begin{itemize}
